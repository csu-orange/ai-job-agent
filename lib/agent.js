...
function resolveProvider() {
  ...
  if (configured === "nvidia") {
    return readEnv("NVIDIA_API_KEY") ? "nvidia" : "local";
  }

  if (configured === "openai") {
    return readEnv("OPENAI_API_KEY") ? "openai" : "local";
  }
...
async function callNvidia({ resume, jobDescription, targetRole, seniority }) {
  ...
  const baseUrl = normalizeNvidiaBaseUrl(readEnv("NVIDIA_BASE_URL", "https://integrate.api.nvidia.com/v1"));
  const model = readEnv("NVIDIA_MODEL", "meta/llama-3.1-70b-instruct");
  const timeoutMs = Number(readEnv("NVIDIA_TIMEOUT_MS", "55000"));
  const maxTokens = Number(readEnv("NVIDIA_MAX_TOKENS", "1200"));
  const stream = readEnv("NVIDIA_STREAM", "true") !== "false";
  ...
  try {
    console.info(`Calling NVIDIA NIM model=${model} baseUrl=${baseUrl} timeoutMs=${timeoutMs} maxTokens=${maxTokens} stream=${stream}`);
    ...
    if (error.name === "AbortError") {
      throw new Error(`NVIDIA API ${timeoutMs}ms 内没有返回。model=${model}，baseUrl=${baseUrl}。请检查模型 ID、额度、区域网络或稍后重试。`);
    }
    throw error;
  } finally {
  ...
  if (!response.ok) {
    const data = await response.json().catch(() => ({}));
    const message = data.error && data.error.message ? data.error.message : response.statusText;
    throw new Error(`NVIDIA API HTTP ${response.status}：${message}`);
  }

  const data = stream ? {} : await response.json().catch(() => ({}));
  const content = stream
    ? await readNvidiaStream(response)
...
async function readNvidiaStream(response) {
  if (!response.body) {
    throw new Error("NVIDIA stream response body is empty.");
  }
  ...
  for await (const chunk of response.body) {
    ...
    for (const rawLine of lines) {
      ...
      try {
        ...
        // Ignore partial or non-JSON SSE lines; the next chunk may complete them.
      }
    }
  }
...
function buildModelPrompt({ resume, jobDescription, targetRole, seniority }) {
  return [
    ...
    jobDescription,
    "",
    "请输出严格 JSON，不要 Markdown。JSON 结构：",
    "{",
    '  "summary": {"matchScore": number, "verdict": string, "strongestFit": string[], "mainRisks": string[], "recommendedStrategy": string},',
    '  "keywordFit": {"matched": string[], "missing": string[], "resumeSignals": string[]},',
    '  "rewritePlan": {"positioning": string, "bulletsToImprove": [{"original": string, "issue": string, "rewrite": string}], "keywordGaps": string[]},',
    '  "tailoredResume": {"headline": string, "summary": string, "coreSkills": string[], "experienceBullets": string[], "projectBullets": string[]},',
    '  "coverLetter": string,',
    '  "interview": {"likelyQuestions": string[], "starStories": string[], "questionsToAsk": string[]},',
    '  "applicationPlan": {"nextActions": string[], "followUpMessage": string},',
    '  "agentTrace": [{"step": string, "status": string, "detail": string}]',
    "}",
    "",
    "每个数组保留 3-5 条，内容具体但简洁。",
    "请完成：匹配度评分、优势与风险、关键词差距、简历改写、定制简历摘要、面试准备、投递行动计划。"
  ].join("\n");
}
...
function normalizeNvidiaBaseUrl(value) {
  return value
    .replace(/\/+$/, "")
